// animations.js - Jiaobei throw animations + stick container animations
...
const Animations = {
  ...
  _buildKeyframes(off, spin, finalFlipX) {
    ...
    const my = spin.midY;
    return [
      { transform: 'translateX(0) translateY(0) rotateX(0deg) rotateY(0deg) rotateZ(0deg)', offset: 0 },
      { transform: `translateX(${off.x * 0.15}px) translateY(${peakY * 0.6}px) rotateX(${spin.totalX * 0.3}deg) rotateY(${my * 0.6}deg) rotateZ(${rz * 0.2}deg)`, offset: 0.2 },
      { transform: `translateX(${off.x * 0.35}px) translateY(${peakY}px) rotateX(${spin.totalX * 0.55}deg) rotateY(${my}deg) rotateZ(${rz * 0.45}deg)`, offset: 0.35 },
      { transform: `translateX(${off.x * 0.65}px) translateY(${off.y * 0.4}px) rotateX(${spin.totalX * 0.8}deg) rotateY(${my * 0.5}deg) rotateZ(${rz * 0.7}deg)`, offset: 0.55 },
      { transform: `translateX(${off.x * 0.9}px) translateY(${off.y + 6}px) rotateX(${finalFlipX}deg) rotateY(0deg) rotateZ(${rz}deg)`, offset: 0.75 },
      { transform: `translateX(${off.x * 0.97}px) translateY(${off.y - 14}px) rotateX(${finalFlipX}deg) rotateY(0deg) rotateZ(${rz}deg)`, offset: 0.85 },
      { transform: `translateX(${off.x}px) translateY(${off.y + 3}px) rotateX(${finalFlipX}deg) rotateY(0deg) rotateZ(${rz}deg)`, offset: 0.93 },
      { transform: `translateX(${off.x}px) translateY(${off.y}px) rotateX(${finalFlipX}deg) rotateY(0deg) rotateZ(${rz}deg)`, offset: 1 },
    ];
  },
  ...
  // --- Stick Container Animations ---
  initStickContainer() {
    const container = document.getElementById('stick-container');
    if (container) container.classList.remove('shaking', 'drawn');
  ...
  startShake() {
    const container = document.getElementById('stick-container');
    if (container) container.classList.add('shaking');
  },

  stopShake() {
    const container = document.getElementById('stick-container');
    if (container) container.classList.remove('shaking');
  },

  async drawStickFromContainer() {
    const container = document.getElementById('stick-container');
    if (!container) return 0;
    container.classList.remove('shaking');

    const sticks = container.querySelectorAll('.stick');
    const risingIndex = Math.floor(Math.random() * sticks.length);
    sticks[risingIndex].classList.add('rising');
    await this._delay(800);
    return getRandomLotNumber();
  },

  // Fallback: click triggers shake + draw together
  async shakeAndDraw() {
    const container = document.getElementById('stick-container');
    if (!container) return 0;
    container.classList.add('shaking');
    await this._delay(1800);
    return this.drawStickFromContainer();
  },

  resetStickContainer() {
    this.initStickContainer();
  },
